...
int main(int argc, char** argv)
{
    ...
    /* Inicialitzar entorn MPI */
    MPI_Init(&argc, &argv);
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &nodos);

    /* --- RESERVA DE MEMORIA DINÁMICA --- */
    /* Aplanamos las matrices a 1D para máximo rendimiento de caché (Fila * N + Columna) */
    ...
    tmd *CD  = malloc(sizeof(tmd) * N * N); // Matriz resultante global (sobredimensionada por seguridad)

    /* TODOS los nodos generan los datos semilla de forma independiente. CERO red. */
    for ( k = 0; k < ND; k++ )
    {
        AD[k].i = rand() % (N - 1);
        AD[k].j = rand() % (N - 1);
    ...
    // TRUCO DE OPTIMIZACIÓN: Ya no necesitamos 'A' para nada más en todo el código.
    // La liberamos ya mismo para recuperar memoria RAM.
    free(A);
    ...
    for ( i = i_ini; i < i_fin; i++ )
    {
        for ( j = 0; j < N; j++ )
        {
            VB_local[j] = B[j * N + i]; // Equivale a B[j][i]
            VC_local[j] = 0;
        }
    ...
    // Comprobamos solo las columnes procesadas por este nodo
    ...
    /* Reducir resultados locales a globales en el ROOT (ÚNICA COMUNICACIÓN) */
